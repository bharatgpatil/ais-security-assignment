...
const mockActionId = require("../../../../mockdata/frontend/sample data/mockId.json");
const mockActionId_7 = require("../../../../mockdata/frontend/sample data/7.json");
...
describe('DashboardComponent', () => {
  ...
  it('getDataForSelectedId should catch error', () => {
    component.path = "path";
    let function_call = spyOn(component, "handleError").and.callThrough();
    spyOn(service, 'getTestDataForActionId').and.callFake(() => {
      return throwError(new Error('Fake error'));
    });
    component.getDataForSelectedId();
    expect(function_call).toHaveBeenCalledTimes(1);
  });

  it('getDataForSelectedId should return data for action 7', () => {
    component.path = "path";
    component.getDataForSelectedId();
    expect(component.actionTestData).toEqual(mockActionId);
  });

  it('computeDataForSelectedAction should set data for action 9', () => {
    component.locMap = mockMap;
    component.actionId = 9;
    component.computeDataForSelectedAction(mockActionId);
    expect(component.widgets).toEqual(mockActionId.widgets);
    expect(component.status).toEqual(mockActionId.SuccessFul);
    expect(component.loc).toEqual(mockMap.get(9));
    expect(component.sourceStateImg).toEqual("");
    expect(component.resultingStateImg).toEqual("");
    expect(component.interactedWidgetId).toEqual(mockActionId.InteractedWidget);
    expect(component.interactedWidgetData).toEqual(mockActionId.interactedWidgetData);
  });

  it('computeDataForSelectedAction should set data for action 7', () => {
    component.locMap = undefined;
    component.computeDataForSelectedAction(mockActionId_7);
    expect(component.widgets).toEqual([]);
    expect(component.interactedWidgetId).toEqual("");
    expect(component.interactedWidgetData).toEqual("");
    expect(component.loc.length).toEqual(0);
  });

  it('handleData should call timeline for defined input', () => {
    let function_call = spyOn(component, "computeData").and.callThrough();
